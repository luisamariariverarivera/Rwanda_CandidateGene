...
setwd("~/Desktop/Rwanda_Epigenetic_Study-main/Rwanda_Epigenetic_Study-main")
...
setwd("~/Desktop/Rwanda_Epigenetic_Study-main/Rwanda_Jama")
...
d <- subset(m_long, select = c("subject2", "Name", "M_Value", "group_factor", "bio_sex", "ace_total", "PC1", "UCSC_RefGene_Group", "crp", 'nlr', "mlr", "deprtotal_promis29", "anxttotal_promis29"))
...
### War Trauma Exposure and ACES interaction
model_contrasts <- data.frame(term = c(),
                              estimate = c(),
                              std.error = c(),
                              statistic = c(),
...
for (i in 1:n_tests) {
  m_temp <- lm(M_Value ~ ace_total*group_factor + bio_sex + PC1, data = dplyr::filter(d, Name == probes[i]))
  #contrast_groups <- avg_comparisons(m_temp, variables = "group_factor")
  summary(m_temp)$coefficients[2,]
  
  temp_df <- comparisons(m_temp, variables = "ace_total", by = "group_factor") %>% as.data.frame() %>% select(c("term", "contrast", "group_factor", "estimate", "std.error", "statistic", "p.value", "conf.low", "conf.high")) %>% mutate(probe = probes[i])
  model_contrasts <- bind_rows(model_contrasts, temp_df)
}
...
write.csv(sig_contrasts, "figures_tables/Significant_CpG_noFDR_ace_group.csv")

# Visualzing interaction
# dplyr::filter(d, Name == probes[i]) %>% ggplot(aes(x = ace_total, y = M_Value, color = group_factor, fill = group_factor)) + geom_point() + geom_smooth(method = "lm")
...
write.csv(sig_contrasts_FDR, "figures_tables/Significant_CpG_FDR_ace_group.csv")
